...
public interface TypeMapper {
  ...
  default Set<BackendKeyDefintionEntity> mapPropertiesDtoToEntity(List<BackendKeyDefinitionDto> definitions) {
    if(Objects.isNull(definitions) || definitions.isEmpty()){
      return Collections.emptySet();
    }
    return definitions.stream().map( definition -> {
      BackendKeyDefintionEntity entity = new BackendKeyDefintionEntity();
  ...
  default List<BackendKeyDefinitionDto> mapPropertiesEntityToDto(Set<BackendKeyDefintionEntity> definitions) {
    if(Objects.isNull(definitions) || definitions.isEmpty()){
      return Collections.emptyList();
    }
    return definitions.stream().map( definition -> {
