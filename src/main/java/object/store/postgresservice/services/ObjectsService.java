...
public record ObjectsService(ObjectsDao objectsDao,TypeService typeService) {
  ...
  public Mono<Map<String,Object>> updateObjectsByTypeId(Mono<Map<String,Object>> monoObject, String typeId){
    ...
    /*typeService.getById(UUID.fromString(typeId)).flatMap( type -> {
      String primaryKey = getPrimaryKeyName(type);
      return objectsDao.getObject(Mono.zip(Mono.just(primaryKey),Mono.just(type))).flatMap( oldObject -> {
        return objectsDao.updateObject(Mono.zip(monoObject,))
      });
    })
    return objectsDao.updateObject(Mono.zip(monoObject,typeService.getById(UUID.fromString(typeId))));*/
  }
